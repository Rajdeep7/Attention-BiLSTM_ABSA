...
parser.add_argument('--vocab-size', type = int, default = 3943)
parser.add_argument('--word-embedding-size', type = int, default = 1024)
parser.add_argument('--aspect-embedding-size', type = int, default = 1024)
...
parser.add_argument('--restore-dir', default = 'semeval16-restaurant-2019-02-14-11-34-40')
parser.add_argument('--meta-file-path', default = 'semeval16-restaurant-16900.meta')
parser.add_argument('--embedding-type', default = 'elmo', choices = ['xavier', 'glove', 'fasttext', 'elmo'])
...
import time
from utils.util import calculate_class_weights, batch_iterator, get_feed_data, load_elmo_emebddings, vocab_to_code
...
def train(config = None, reporter = None):
    ...
    with tf.Session(config = config) as session:
        ...
        _, saver, ops = model_elmo.get_model(session = session,
                                             args = args,
                                             restore_only = False)
...
def evaluate(session, ops, dataset):
    ...
    for x1, x2, y in batch_iterator(dataset, args.batch_size, 1):
        # get feed_dicts
        fd = get_feed_data(x1, x2, y, is_training = False, args = args, elmo = elmo)
        ...
        if args.mode == 'test':
            aspect_codes = []
            for words in x1:
                aspect_codes.append(vocab_to_code(words))
            eval_results = evaluation_metrics(aspect_codes, fd[PADDED_REVIEWS_TENSOR_NAME],
...
def test(best_step):
    ...
    with tf.Session(config = config) as session:
        _, _, ops = model_elmo.get_model(session = session,
                                         args = args,
                                         restore_only = True)
