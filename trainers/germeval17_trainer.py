...
parser.add_argument('--task', default = 'germeval17', choices = ['germeval17'])
...
parser.add_argument('--vocab-size', type = int, default = 85683, choices = [72115, 85683, 93776])
...
parser.add_argument('--word-output-size', type = int, default = 500)
...
parser.add_argument('--restore-dir', default = 'germeval17-2018-12-31-14-15-24')
parser.add_argument('--meta-file-path', default = 'germeval17-6000.meta')
...
from utils.data_util import train_loader, test_loader, val_loader, write_binary
...
from preprocessing.germEval17_data_processing import GERMEVAL_ASPECT_WORD_INDEX_MAP
from models import model_germeval17
import numpy as np
import ray
from ray import tune
from config.tensor_names import PADDED_REVIEWS_TENSOR_NAME, SENTENCE_MASK_TENSOR_NAME, PADDED_LABELS_TENSOR_NAME, \
    ASPECTS_TENSOR_NAME
from utils.evaluation_util import evaluation_metrics
from utils.prediction_formatter import write_prediction_to_xml
...
def train(config = None, reporter = None):
    ...
    with tf.Session(config = config) as session:
        ...
        _, saver, ops = model_germeval17.get_model(session = session,
                                                   args = args,
                                                   restore_only = False)
        ...
        for i, (x1, x2, y) in enumerate(batch_iterator(train_loader(args.epochs), args.batch_size)):
            ...
            if step % args.print_frequency == 0:
                ...
                print('step %s, loss=%s, accuracy=%s, f1_score=%s, t=%s, inputs=%s' % (
                    step, loss, accuracy, f1_score, round(td, 2),
                    fd[PADDED_REVIEWS_TENSOR_NAME].shape))
