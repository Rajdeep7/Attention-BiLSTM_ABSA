...
parser.add_argument('--task', default = 'semeval16-restaurant', choices = ['semeval16-restaurant', 'semeval16-laptops'])
...
parser.add_argument('--vocab-size', type = int, default = 8705, choices = [8705, 5511, 5093, 4308, 4143])
...
parser.add_argument('--restore-dir', default = 'semeval16-restaurant-2019-01-05-10-09-31')
parser.add_argument('--meta-file-path', default = 'semeval16-restaurant-100.meta')
parser.add_argument('--previous-restore-dir', default = 'previous_laptops')
parser.add_argument('--previous-meta-file-path', default = 'semeval16-laptops-148400.meta')
...
parser.add_argument('--load-graph', type = bool, default = False)
...
from models import cnn_progressive_nets as model
import numpy as np
import ray
...
def train(config = None, reporter = None):
    ...
    with tf.Session(config = config) as session:
        ...
        _, saver, ops = model.get_model(session = session, args = args, restore_only = False)
        ...
        for i, (x1, x2, y) in enumerate(batch_iterator(train_loader(args.epochs), args.batch_size)):
            ...
            word_level_inputs, aspect_embedded_encoder_output, aspect_embedded_sentence_inputs, birnn_output = session.run(
                [previous_ops['word_level_inputs'], previous_ops['aspect_embedded_encoder_output'],
                 previous_ops['aspect_embedded_sentence_inputs'], previous_ops['birnn_output']], previous_fd
            )

            fd[PREFIX + PREVIOUS_WORD_LEVEL_INPUTS_TENSOR_NAME] = word_level_inputs
            fd[PREFIX + PREVIOUS_ASPECT_EMBEDDED_ENCODER_OUTPUT_TENSOR_NAME] = aspect_embedded_encoder_output
            fd[PREFIX + PREVIOUS_ASPECT_EMBEDDED_SENTENCE_INPUTS_TENSOR_NAME] = aspect_embedded_sentence_inputs
            fd[PREFIX + PREVIOUS_SENTENCE_ENCODER_OUTPUT_TENSOR_NAME] = birnn_output
            fd[PREFIX + ASPECTS_TENSOR_NAME] = fd[ASPECTS_TENSOR_NAME]
            fd[PREFIX + PADDED_REVIEWS_TENSOR_NAME] = fd[PADDED_REVIEWS_TENSOR_NAME]
            fd[PREFIX + ACTUAL_SENTENCE_COUNT_TENSOR_NAME] = fd[ACTUAL_SENTENCE_COUNT_TENSOR_NAME]
            fd[PREFIX + ACTUAL_WORD_COUNT_TENSOR_NAME] = fd[ACTUAL_WORD_COUNT_TENSOR_NAME]
            ...
            if step % args.print_frequency == 0:
                ...
                print('step %s, loss=%s, accuracy=%s, f1_score=%s, t=%s, inputs=%s' % (
                    step, loss, accuracy, f1_score, round(td, 2), fd[PREFIX+PADDED_REVIEWS_TENSOR_NAME].shape))
            if step != 0 and step % args.eval_frequency == 0:
                # run validation
                val_results = evaluate(session = session, ops = ops, previous_ops = previous_ops,
...
def evaluate(session, ops, previous_ops, dataset):
    ...
    for x1, x2, y in batch_iterator(dataset, args.batch_size, 1):
        ...
        if args.mode == 'test':
            eval_results = evaluation_metrics(fd[PREFIX+ASPECTS_TENSOR_NAME], fd[PREFIX+PADDED_REVIEWS_TENSOR_NAME],
                                              fd[PREFIX+PADDED_LABELS_TENSOR_NAME], masked_prediction, aspect_word_index_map)
...
def test(best_step):
    ...
    with tf.Session(config = config) as session:
        ...
        # get previous models for transfer learning
        # previous_ops = model.get_model(session = session, args = args, restore_only = True)
