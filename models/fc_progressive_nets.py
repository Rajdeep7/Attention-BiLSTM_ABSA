...
class Model:
    def __init__(self, args, word_cell, sentence_cell):
        ...
        with tf.variable_scope('current_absa'):
            ...
            # These are inputs from the previous models or columns
            ...
            # These are U matrix from the progressive nets paper
            # self.previous_word_level_inputs_connector = tf.get_variable(name = 'previous_word_level_inputs_connector',
            #                                                             shape = (None, None,
            #                                                                      self.args.word_embedding_size + self.args.aspect_embedding_size),
            ...
            #                                                             trainable = True)
            #
            # self.previous_aspect_embedded_encoder_output_connector = tf.get_variable(
            #     name = 'previous_aspect_embedded_encoder_output_connector',
            #     shape = (None, None, 2 * args.word_cell_units + args.aspect_embedding_size),
            #     initializer = layers.xavier_initializer(),
            #     dtype = tf.float32,
            #     trainable = True)
            #
            # self.previous_aspect_embedded_sentence_inputs_connector = tf.get_variable(
            #     name = 'previous_aspect_embedded_sentence_inputs_connector',
            #     shape = (None, None, args.word_output_size + args.aspect_embedding_size),
            #     initializer = layers.xavier_initializer(),
            #     dtype = tf.float32,
            #     trainable = True)
            #
            # self.previous_birnn_output_connector = tf.get_variable(
            #     name = 'previous_birnn_output_connector',
            #     shape = (None, None, 2 * args.sentence_cell_units),
            #     initializer = layers.xavier_initializer(),
            #     dtype = tf.float32,
            #     trainable = True)
    ...
    def _init_model_body(self):
        ...
        with tf.variable_scope('word'):
            ...
            with tf.variable_scope('word_bilstm'):
                ...
                output = TimeDistributed(
                    Dense(self.args.word_embedding_size + self.args.aspect_embedding_size, activation = 'relu'),
                    input_shape = (self.batch_size * self.max_sentence_count,
                                   self.max_word_count,
                                   self.args.word_embedding_size + self.args.aspect_embedding_size))(
                    self.previous_word_level_inputs)
                # output = self.previous_word_level_inputs_connector * output
                self.word_level_inputs = self.word_level_inputs + output
                self.word_level_inputs.set_shape([None,
                                                  None,
                                                  self.args.word_embedding_size + self.args.aspect_embedding_size])
            ...
            with tf.variable_scope('word_attention'):
                if self.args.use_attention:
                    ...
                    output = TimeDistributed(
                        Dense(2 * self.args.word_cell_units + self.args.aspect_embedding_size, activation = 'relu'),
                        input_shape = (self.batch_size * self.max_sentence_count,
                                       self.max_word_count,
                                       2 * self.args.word_cell_units + self.args.aspect_embedding_size))(
                        self.previous_aspect_embedded_encoder_output)
                    # output = self.previous_aspect_embedded_encoder_output_connector * output
                    self.aspect_embedded_encoder_output = self.aspect_embedded_encoder_output + output
                    self.aspect_embedded_encoder_output.set_shape([None,
                                                                   None,
                                                                   2 * self.args.word_cell_units + self.args.aspect_embedding_size])
        ...
        with tf.variable_scope('sentence'):
            ...
            with tf.variable_scope('sentence_aspect_concatenate'):
                ...
                output = TimeDistributed(
                    Dense(self.args.word_output_size + self.args.aspect_embedding_size, activation = 'relu'),
                    input_shape = (self.batch_size, self.max_sentence_count,
                                   self.args.word_output_size + self.args.aspect_embedding_size))(
                    self.previous_aspect_embedded_sentence_inputs)
                # output = self.previous_aspect_embedded_sentence_inputs_connector * output
                self.aspect_embedded_sentence_inputs = self.aspect_embedded_sentence_inputs + output
                self.aspect_embedded_sentence_inputs.set_shape([None,
                                                                None,
                                                                self.args.word_output_size + self.args.aspect_embedding_size])
            ...
            with tf.variable_scope('sentence_bilstm'):
                ...
                output = TimeDistributed(
                    Dense(2 * self.args.sentence_cell_units, activation = 'relu'),
                    input_shape = (self.batch_size, self.max_sentence_count,
                                   2 * self.args.sentence_cell_units))(
                    self.previous_birnn_output)
                # output = self.previous_birnn_output_connector * output
                self.sentence_encoder_output = self.sentence_encoder_output + output
                self.sentence_encoder_output.set_shape([None,
                                                        None,
                                                        2 * self.args.sentence_cell_units])
